    ...
    function validateField ( field ) {
        ...
        if ( field.attr("name") === "subject" ) {
          if ( !validateLength( value, 3 ) ) {
                error = true;
                errorText += '<i class="fa fa-info-circle"></i> Enter correct Subject Name!<br>';
                jQuery('input[name="subject"]').addClass('input-error');
          } else {
              jQuery('input[name="subject"]').removeClass('input-error');
          }
        }
    ...
    jQuery('.contact-form').on('submit', function(e) {
        ...
        jQuery.ajax({
            ...
            success: function( data ) {
                if (data) {
                    jQuery('body').find('.contact-message').html('<div class="alert alert-dismissible alert-success fade show" role="alert"> <strong>'+ data +'</strong><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button> </div> ');
                }
            ...
            error: function( data ) {
                if (data.responseText !== '') {
                    jQuery('body').find('.contact-message').html('<div class="alert alert-dismissible alert-danger fade show" role="alert"> <strong>Error:</strong> '+ data.responseText +'<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button> </div> ');
                } else {
                   jQuery('body').find('.contact-message').html('<div class="alert alert-dismissible alert-danger fade show" role="alert"> <strong>Error:</strong> Oops! An error occured and your message could not be sent. <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button> </div> ');
                }
            }
        });
    });
